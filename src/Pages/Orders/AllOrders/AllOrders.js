...
const AllOrders = () => {
    ...
    return (
        <div>
            {
                order ?
                    order.map(dress => <div key={Math.random()} className='md:flex justify-between bg-gray-50 rounded px-4 m-1'>
                        <div className='hidden md:block w-16 h-16'>
                            <img className='w-full h-full rounded' src={dress.image} alt={dress.name} />
                        </div>
                        <h1 className='md:w-2/6 text-2xl font-bold my-auto'>{dress.name}</h1>
                        <p className='md:w-2/6 text-lg font-semibold my-auto'>Renting for <span className='text-gray-500'>${dress.price}</span></p>
                        <p className='md:w-3/12 text-lg font-semibold my-auto'>Rented by <span className='text-gray-500'>{dress.client} ({dress.clientEmail})</span></p>
                        <p className='md:w-3/12 text-lg font-semibold my-auto'>Rented from <span className='text-gray-500'>{dress.address}</span></p>
                        {
                            dress.pending ?
                                <div className='md:flex my-auto md:w-3/12'>
                                    <p className=' text-lg font-semibold mx-1'>Pending for Delivery</p>
                                    <button onClick={() => handleAuthorizeDelivery(dress)} className='text-white bg-green-700 hover:bg-red-500 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center'>Authorize</button>
                                </div> :
